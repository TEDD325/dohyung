...
learning_rate = 0.0001 * num_workers (TF에서 지정한 default값은 0.001)
self.global_mean_loss와 self.global_max_mean_score 두개를 모두 고려하여 교환
"""
...
class DQNAgent:
    ...
    def start_rl(self, socket):
        ...
        for episode in range(self.max_episodes):
            ...
            if self.global_max_mean_score < mean_score and self.global_mean_loss > mean_loss: #!
            ...
            elif self.global_max_mean_score > mean_score and self.global_mean_loss < mean_loss:
            # else:
                send_weights = False
    ...
    def send_episode_info_and_adapt_best_weights(self, socket, loss, score, mean_score, send_weights):
        ...
        if episode_ack_msg["type"] == "episode_ack":
            if transfer:
                ...
                global_mean_loss = episode_ack_msg["global_mean_loss"]
                ...
                if len(best_weights) > 0 and not send_weights:
                    ...
                    self.global_mean_loss = global_mean_loss
                    ...
                    msg = ">>> Worker {0}: Set New Best Weights to Local Model!!! - global_max_score: {1} , global_mean_loss: {2}".format(
                        self.worker_idx,
                        self.global_max_mean_score,
                        self.global_mean_loss
                    )
                    self.logger.info(msg)
                    if verbose: print(msg)
                ...
                if multi_dqn.continue_loop:
                    if transfer:
                        ...
                        episode_ack_msg = {
                            ...
                            "global_mean_loss": multi_dqn.global_mean_score,
