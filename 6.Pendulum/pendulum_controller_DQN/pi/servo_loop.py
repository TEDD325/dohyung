...
class QubeServo2:
    ...
    def __data_conversion(self, data):
        ...
        motor_position = encoder0 * (-2.0 * PI / 2048.0) # motor radian
        ...
        # convert the arm encoder counts to angle theta in radians
        pendulum_angle = encoder1 * (2.0 * PI / 2048.0) - PI # pendulum radian
    ...
    def limit_check(self):
        ...
        if motor_radian > PI / 1.4 or motor_radian < -PI / 2: # motor_radian > 2.244 or motor_radian < -1.571
    ...
    def manual_swing_up(self):
        ...
        while True:
            ...
            if currentTime - previousTime >= UNIT_TIME:
                ...
                if 0.0 <= abs(pendulum_radian) <= PI * 11.0 / 180.0: # PI * 11.0 / 180.0 =: 0.2
                    # -11 degree ~ +11 degree
                    break

                angular_variation = (pendulum_radian - last_pendulum_radian)
                # angular variation filtering
                if angular_variation > 2.5: # +143.2 degree
                    angular_variation -= PI * 2 # PI * 2 =: 6.3 radian
                    # -217.7 degree, -223.5 degree, ... # 각도의 크기에 따라 penulum에 가할 voltage값이 결정된다.
                elif angular_variation < -2.5:
                    angular_variation += PI * 2

                pendulum_angular_velocity = angular_variation / UNIT_TIME # 각속도
                # if angular_variation==3.7, then pendulum_angular_velocity==3700
                ...
                if abs(pendulum_angular_velocity) > 25:
                    voltage /= int(10 * np.log(abs(pendulum_angular_velocity))) # if pendulum_angular_velocity==3.7 and voltage==80.0, then voltage=:1.308

                if PI >= abs(pendulum_radian) >= PI * 90.0 / 180.0: # PI * 90.0 / 180.0 =: 1.5708
                    if pendulum_radian >= 0:
                        pendulum_radian = PI - pendulum_radian
                    else:
                        pendulum_radian = - PI + abs(pendulum_radian)

                    if pendulum_angular_velocity == 0:
                        if random.random() < 0.5:
                            motorPWM = int(-2 * math.cos(pendulum_radian) * voltage) # if pendulum_radian==0.15 and voltage==1.308, then motorPWM =:-2.58663
                        else:
    ...
    def manual_balance(self):
        ...
        while count < 1500 / 5:
            ...
            if currentTime - previousTime >= UNIT_TIME*5:
                ...
                if abs(alpha) <= (30.0 * PI / 180.0): # 30.0 * PI / 180.0 =: 0.5236
