...
if __name__ == '__main__':
    ...
    # loss
    loss_trials = 10
    losses = deque(maxlen=loss_trials)

    logger.setLevel(logger.ERROR)
    env = gym.make(args.env_id)

    outdir = "./dqn_advanced/dqn-%s" % args.env_id
    if args.ddqn:
        outdir = "./dqn_advanced/ddqn-%s" % args.env_id
    ...
    for episode in range(episode_count):
        ...
        if len(agent.memory) >= batch_size:
            ...
            if episode % loss_trials == 0:
                print("Episode {0}: Mean Loss = {1}".format(
                    episode,
                    mean_loss
                ))
        ...
        if episode % win_trials == 0:
            print(">>> Episode {0}: Mean score = {1} in {2} episodes".format(
                episode,
                mean_score,
                win_trials
            ))
        ...
        if mean_score >= win_reward[args.env_id] and episode >= win_trials:
            print("*** Solved in episode {0}: Mean score = {1} in {2} episodes".format(
                episode,
                mean_score,
                win_trials
            ))
