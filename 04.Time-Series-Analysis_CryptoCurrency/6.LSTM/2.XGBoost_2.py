...
def vanilla_LSTM(window_size, units_1, units_2, n_state_units=32, activation_1='softmax', activation_2='relu', optimizer='adam'):
    ...
    config.gpu_options.per_process_gpu_memory_fraction = 1.0  # what portion of gpu to use
    ...
    for coin in coin_list2:
        if (os.path.isfile(pickle_result_dir_path + \
            ...
            print(coin + "_" + \
                  ...
                  str(idx_gap) + "_" + \
                  str(idx_margin_rate) + \
                  "_result.csv FILE ALREADY EXIST.")
            continue
        else:
...
#                                        batch_size=512,
#                                        callbacks=[stopper])
                grid_result = random_search.fit(   X_train_scaled,
                                                   y_train,
                                                   validation_data=(X_test_scaled,
                                                                    y_test),
            ...
            if (os.path.isfile(pickle_result_dir_path + \
                ...
                with open(pickle_result_for_email_transfer + \
                    ...
                    sending_email_for_file_transfer(evaluate_result, pickle_result_for_email_transfer + \
                                                                       coin + "_" + \
                                                                       str(idx_time_unit) + "_" + \
                                                                       str(idx_window_size) + "_" + \
                                                                       str(idx_gap) + "_" + \
