...
from test import SpecifiedParameter

# stopper = EarlyStopping(monitor='loss', patience=3, verbose=1, min_delta=0.01)
stopper = keras.callbacks.EarlyStopping(monitor='recall', patience=0, verbose=1, mode='auto', min_delta=0)
...
def create_model_LSTM(window_size,
    ...
    model = Sequential()
    for i in range(2):
        model.add(LSTM(units=n_state_units,
                       input_shape=(window_size, 32),
                       stateful=True,
                       batch_input_shape=(1, window_size, 32),
    ...
    model.compile(loss=keras.losses.categorical_crossentropy,
                  optimizer=optimizer,
                  metrics=[_f1_score, 'accuracy', precision, recall])
...
def run_Model(run_test, epochs, cv, n_iter, _GPU, model_info_):
    if run_test == True:
        ...
        if model_info_ == "window_size_standard":
            model_info[model_info_] = SpecifiedParameter.machine
...
if __name__ == "__main__":
    ...
    run_Model(run_test=run_test,
              ...
              n_iter=n_iter,
              _GPU=_GPU,
              model_info_=SpecifiedParameter.model_info)
